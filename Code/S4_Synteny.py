...
parser.add_argument('-b', '--blastp_dir', default = '../data/blastp', help = 'folder that has blastp files')
parser.add_argument('-g', '--gff_dir', default = '../data/edited_gff', help = 'folder containing edited gff files')
...
parser.add_argument('-d', '--doc_fol', default = '../doc/synteny', help = 'folder that will have the job submission files')
parser.add_argument('-p', '--paml_doc', default = '../doc/PairwisePaml', help = 'has control files') 
parser.add_argument('-o', '--out_fol', default = '../data/synteny', help = 'folder that will have the output')
...
def ParseColFl(ColinFl):
    """ sort collinear segments according to their score, rank them.
...
def PairwiseAnalysis(ColDf, sp1, sp2, sp1Id, sp2Id, PepFlSp1, PepFlSp2, TransFlSp1, TransFlSp2):
    ...
    for indx, row in ColDf[[sp1Id, sp2Id]].iterrows():#iterate through the rows of the ColinDf to extract gene pairs
        ...
        if Sp1Identity < 85 or Sp2Identity < 85: #if not the best match drop the column
...
for sp1 in SpList:
    ...
    #duplicate gene classifier : 0 - singleton, 1 - dispersed, 2 - proximal, 3 - tandem, 4 -  WGD/segmental.
    os.popen(r"/groups/prodiv/software/MCScanX/duplicate_gene_classifier {}/{}".format(OutSynDataDir, sp1Id)).read()
