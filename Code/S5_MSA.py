...
# -*- coding: utf-8 -*-
"""
Step-5 Orthlog family selection and Multiple Sequence Alignment
"""
import time
...
parser.add_argument('-S', '--syn_dir', default = '../data/synteny', help = 'folder that has blastp files')
...
def DropDuplicates(FlName, FocalId, OthSpId):
    """Sort the gene pairs by the e-value and the length of their syntenic 
...
def MSA(CommonDf):
    ...
    for indx, row in CommonDf.iterrows():
        ...
        CommonDf.at[indx,aaSp1] = len(PepSp1[indx].seq)
        CommonDf.at[indx,aaSp2] = len(PepSp2[row[Sp2Id]].seq)
        CommonDf.at[indx,aaSp3] = len(PepSp3[row[Sp3Id]].seq)
        CommonDf.at[indx,aaSp4] = len(PepSp4[row[Sp4Id]].seq)
